...
import 'package:assalam/data/services/claim_reward/claim_vip_premium_reward.dart';
...
class VipPremiumReferencePage extends StatefulWidget {
  const VipPremiumReferencePage({super.key});

  @override
  State<VipPremiumReferencePage> createState() => _VipPremiumReferencePageState();
}

class _VipPremiumReferencePageState extends State<VipPremiumReferencePage> {
  ...
  final claimVipRewardData =  ClaimVipPremiumRewardGetData();
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title: FutureBuilder<Translation>(
          future: translator.translate('VIP Premium Referral', from: 'auto', to: targetLanguage),
                    ...
                    child: Row(
                      ...
                      children: [
                        Column(
                          ...
                          crossAxisAlignment: CrossAxisAlignment.center,
                          children: [
                            userProfileDataModel?.user!.subVip == '1' ? FutureBuilder<Translation>(
                              future: translator.translate('You Are Subscribed\nto VIP Premium \n\$10 Package', from: 'auto', to: targetLanguage),
                              builder: (context, snapshot) {
                                if (snapshot.hasData) {
                                  final translatedText = snapshot.data!.text;
                            ...
                            ) : Text('You are not Subscribed\n VIP Premium Package!', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w500, color: Colors.white)),
                          ],
                        ),
                        ...
                        Column(
                          ...
                          children: [
                            InkWell(
                              onTap: () {
                                Share.share(userProfileDataModel!.referLinkVip!);
                              },
                              child: Container(
                                // padding: Edgins,
                                alignment: Alignment.center,
                                ...
                                //   future: translator.translate('Share VIP Premium Link', from: 'auto', to: targetLanguage),
                                //   builder: (context, snapshot) {
                                //     if (snapshot.hasData) {
                                //       final translatedText = snapshot.data!.text;
                                //       return Text(translatedText, style: TextStyle(fontSize: 13, fontWeight: FontWeight.w500, color: TColors.primaryColor),textAlign: TextAlign.center,);
                                //     } else if (snapshot.hasError) {
                                //       return Text(
                  ...
                  child: InkWell(
                    onTap: () async {
                      if(userProfileDataModel?.user!.subPremium == '1'){
                        await claimVipRewardData.ClaimVipPremiumRewardData();
                        showDialog(context: context, builder: (context) {
                          return AlertDialog(
                            content: Column(
                              ...
                              children: [
                                SizedBox(height:size.height/78.8),
                                Text(claimVipRewardData.claimVipPremiumRewardData['message'], style: TextStyle(fontWeight: FontWeight.w500, fontSize: 18, color: TColors.primaryColor)),
                              ],
                            ),
                          );
                        },);
                    ...
                    child: Container(
                      ...
                      child: Text('Claim VIP Reward', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w500,color: Colors.white),),
                    ),
                  ),
                ),

                Container(
                  margin: EdgeInsets.only(top: size.height /3.28, left: size.width/18, right: size.width/18),
                  ...
                  child: Padding(
                    padding: EdgeInsets.symmetric(vertical: size.height/78.8),
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.center,
                      children: [
